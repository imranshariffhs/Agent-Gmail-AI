...
def cleanup_resources():
    ...
    try:
        if _event_loop is not None and not _event_loop.is_closed():
            ...
            cleanup_loop = (
                _event_loop if not _event_loop.is_closed() else asyncio.new_event_loop()
            )
...
def validate_gemini_api_key(api_key):
    ...
    try:
        ...
        if response.status_code == 401:
            raise ValueError(
                "Invalid or expired API key. Please renew your Gemini API key."
            )
            ...
            raise ValueError(
                f"API key validation failed with status code: {response.status_code}"
            )
...
def initialize_log_file():
    ...
    try:
        if not LOG_FILE.exists():
            ...
            with pd.ExcelWriter(
                LOG_FILE, engine="openpyxl", datetime_format="YYYY-MM-DD HH:MM:SS"
            ) as writer:
...
def is_file_already_downloaded(log_df, filename, file_data, email_id, thread_id=None):
    ...
    exact_matches = log_df[
        log_df["file_hashes"].apply(
            lambda x: file_hash in str(x).split(",") if pd.notna(x) else False
        )
    ]
    ...
    if thread_id:
        thread_matches = log_df[
            ...
            & (
                log_df["attachment_names"].apply(
                    lambda x: filename in str(x).split(",") if pd.notna(x) else False
                )
            )
        ]
    ...
    unique_matches = log_df[
        log_df["unique_file_ids"].apply(
            lambda x: unique_file_id in str(x).split(",") if pd.notna(x) else False
        )
    ]
    ...
    try:
        ...
        if "email_id" in new_row:
            ...
            if not existing_email.empty:
                ...
                existing_count = (
                    existing_email["count_download"].iloc[0]
                    if "count_download" in existing_email
                    else 0
                )
                existing_names = set()
                if "original_filenames" in existing_email and pd.notna(
                    existing_email["original_filenames"].iloc[0]
                ):
                    existing_names = set(
                        str(existing_email["original_filenames"].iloc[0]).split(",")
                    )
                ...
                if "original_filenames" in new_row:
                    if isinstance(new_row["original_filenames"], list):
                        new_names = set(
                            str(name) for name in new_row["original_filenames"]
                        )
                ...
                for field in [
                    ...
                    if field in existing_email and pd.notna(
                        existing_email[field].iloc[0]
                    ):
                        existing_values = set(
                            str(existing_email[field].iloc[0]).split(",")
                        )
                        new_values = set(str(new_row.get(field, "")).split(","))
                        new_row[field] = ",".join(
                            sorted(existing_values.union(new_values))
                        )
            else:
                ...
                if "original_filenames" in new_row:
                    if isinstance(new_row["original_filenames"], list):
                        ...
                        new_row["list_name_count"] = len(
                            set(new_row["original_filenames"])
                        )
                    elif isinstance(new_row["original_filenames"], str):
...
def monitor_gmail_for_new_attachments_with_logging() -> str:
    ...
    try:
        ...
        query = (
            f"has:attachment in:inbox -from:me after:{yesterday.strftime('%Y/%m/%d')}"
        )
        ...
        for msg in messages:
            try:
                ...
                if classification not in ["ENQUIRY", "QUOTATION"]:
                    skip_reason = (
                        f"Not a valid enquiry or quotation email (got {classification})"
                    )
                    ...
                    if filename in document_attachments:
                        try:
                            ...
                            if "body" not in part or "attachmentId" not in part["body"]:
                                logger.info(
                                    "Skipping %s: Invalid attachment data", filename
                                )
                                continue
                            ...
                            is_duplicate, reason = is_file_already_downloaded(
                                log_df, filename, file_data, msg["id"], thread_id
                            )
                            ...
                            final_filename = (
                                f"{classification.lower()}_{timestamp}_{filename}"
                            )
        ...
        return (
            ...
            else "📭 No new document attachments found in recent emails."
        )
        ...
        for msg in messages:
            try:
                ...
                if not parts:
                    parts = (
                        [msg_data["payload"]]
                        if msg_data["payload"].get("filename")
                        else []
                    )
                ...
                if successfully_downloaded:
                    ...
                    new_row = {
                        ...
                        "res_status": ",".join(
                            res_status_list
                        ),  # One 'pending' for each file
                        "res_path": ",".join(
                            res_path_list
                        ),  # One empty path for each file
                        "duplicate_check_date": current_time,
                        "duplicate_status": "unique",
                    }
...
def view_download_log() -> str:
    ...
    try:
        ...
        total_files = (
            log_df["count_download"].sum() if "count_download" in log_df.columns else 0
        )
        unique_threads = (
            log_df["thread_id"].nunique() if "thread_id" in log_df.columns else 0
        )
        ...
        if "download_date" in log_df.columns:
            log_df["download_date"] = pd.to_datetime(
                log_df["download_date"], errors="coerce"
            )
            recent = log_df[
                log_df["download_date"] > (datetime.now() - timedelta(days=7))
            ]
    ...
    try:
        ...
        try:
            ...
            return {
                ...
                "error": None
                if all_successful
                else "Some steps failed, see output for details",
            }
