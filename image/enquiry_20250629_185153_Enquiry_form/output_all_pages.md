# PDF Extraction Summary
                                **File:** /mnt/c/Users/Imran/OneDrive - Ahana Systems and Solutions (P) Ltd/Desktop/Demo/steer_document_processing_poc/demo_app/backend/Agent_AI/download_email/enquiry_20250629_185153_Enquiry_form.pdf
                                **Total Pages:** 8
                                **Successfully Processed:** 8
                                **Failed:** 0
                                **Extraction Date:** 2025-06-30 07:34:13
                                ---
                        ## page_1.png

```markdown
# STEER

SEPL/SYS-TSE-EF/F-01 Rev:No: 00 / 11-08-2020
Co-rotating Twin screw extruder Enquiry Form

## Please tell us about your company details

**Name of the client:** Gulf Additives LLC
**Building No:**
**Street:** Road No. 4 C
**City/Town:** Rusayd
**State:** Muscat
**Pin code:** 117
**Telephone:** +95824448258
**Email:** sushilkumarb@otegroup.com
**Website:** Gulf Additives.com

## Site Information

**Ambient temperature at site:** Min 25 deg C Max 45 deg C
**Altitude:** Meters above sea level
**Relative humidity at site:** Min 25 % Max 55 %
**Power voltage/Hz:** 440 Voltage 50 Hz
**3 Phase, 1 neutral:** Yes
**Cooling water available:** Yes
**Raw water available:** Yes
**Pneumatic air available:** Yes
**Place of machine installation:** Non hazardous
**Please specify IP protection required:** Standard IP55
unselect[] Main Motor
unselect[] Lube oil pump
unselect[] Small motors
unselect[] Junction box
unselect[] Heaters and Thermocouple
unselect[] PLC panel
unselect[] Drive panel

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_2.png

```markdown
# STEER

Co-rotating Twin screw extruder Enquiry Form

# Scope of supply and work

unselect[] Only extruder
select[X] Any addition please select below,
**Extruder supplier scope:**
unselect[] Mezzanine floor
unselect[] Refill hoppers
unselect[] Conveying Gravimetric feeder
select[X] Volumetric feeder
unselect[] Screen changer
unselect[] Melt pump
unselect[] Classifier
select[X] Water bath
select[X] Air knife Pelletizer
select[X] Under water pelletizer
unselect[] FG storage and conveying
unselect[] 25Kg/Jumbo bag packing and sealing

**Extruder supplier work: -**
unselect[] Complete plant engineering
unselect[] Cable supply, laying and termination
unselect[] Upstream and downstream equipment supervision work
unselect[] Utility piping supply and connection
unselect[] De-dusting system and ducting work

Note: - Manpower and material handling resources need to be arranged by customer.

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

## page_3.png

```markdown
# STEER
Co-rotating Twin screw extruder Enquiry Form

# Application details

**Basic recipe details: :**

**End Application details:**
- select[X] Industrial application
- unselect[] Automotive
- select[X] Home appliance
- unselect[] Housing and construction
- unselect[] Electrical and electronics
- unselect[] Packaging
- unselect[] Specialty

**End product usage:**
- select[X] Extrusion
- unselect[] Molding

**End product form:**
- select[X] Granules
- unselect[] Sheet

**End product testing method:**
- unselect[] ASTM
- select[X] ISO
- unselect[] Will be provided before contract

**Bench mark available:**
- unselect[] Yes
- unselect[] No
- unselect[] Will be provided before contract

**If yes, please specify:**
**MFI:**
**Impact:**
**Tensile:**
**Flexural:**
**Flammability:**
- unselect[] Will be provided before contract

**If any other, please specify:**
Muffel Furness

**Lab trial required:**
- select[X] Yes
- unselect[] No

**If yes, please specify location for the lab trial:**
- select[X] STEER India
- unselect[] STEER Japan
- unselect[] STEER china
- unselect[] STEER America

**Willing to send sample along with MSDS to specified location:**
- select[X] Yes
- unselect[] No

INDIA | CHINA| JAPAN | USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_4.png

```markdown
# STEER

Co-rotating Twin screw extruder Enquiry Form

# Raw material details

## Formulation-1

| SI | Raw material Name | Bulk density gram/cc | Physical form(Flakes/granules/powder/liquid) | Size in mm/micron/Cps | Feed rate % |
|---|---|---|---|---|---|
| select[X] | LLDPE | 0.590 | Powder |  | 22.4 |
| 2 | CA5 | 0.320 | Powder |  | 0.5 |
| 3 | a/o | - | Powder |  | 0.1 |
| 4 | CaCO3 | 1.3 | Powder |  | 52 |
| select[X] | TiO2 | 3.65 | Powder |  | 25 |
| 6 |  |  |  |  |  |
| 7 |  |  |  |  |  |

## Formulation-2

| SI | Raw material Name | Bulk density gram/cc | Physical form(Flakes/granules/powder/liquid) | Size in mm/micron/Cps | Feed rate % |
|---|---|---|---|---|---|
| 1 | LLDPE | 0.590 | Powder |  | 37.65 |
| 2 | CA5 | 0.32 | Powder |  | 0.5 |
| 3 | a/o | - | Powder |  | 0.1 |
| 4 | CaCO3 | 1.3 | Powder |  | 30 |
| 5 | Red 53:1 | 1.5 | Powder |  | 18.25 |
| 6 | Red 57:1 | 1.5 | Powder |  | 3.5 |
| 7 | Wax | 0.6 | Powder |  | 10 |

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

## page_5.png

```markdown
# STEER

Co-rotating Twin screw extruder Enquiry Form

# Raw material details

## Formulation-3

| SI | Raw material Name | Bulk density gram/cc | Physical form(Flakes/granules/powder/liquid) | Size in mm/micron/ Cps | Feed rate % |
|---|---|---|---|---|---|
| 1 | LLDPE |  | Powder |  | 40.4 |
| 2 | CA5 |  | Powder |  | 0.5 |
| 3 | A/O |  | Powder |  | 0.1 |
| 4 | CaCO3 |  | Powder |  | 40 |
| 5 | PY17 (Yellow Pigment) |  | Powder |  | 7 |
| 6 | PY13 (Yellow Pigment) |  | Powder |  | 7 |
| 7 | Wax |  | Powder |  | 5 |

**Raw material storage:** select[X] Inside warehouse
unselect[] Bulk silo

**Type of screening required:**
unselect[] Magnetic separator
unselect[] Metal separator

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

## page_6.png

```markdown
# STEER

## Co-rotating Twin screw extruder Enquiry Form

# Extruder details

**Extruder specification required from:** select[X] STEER unselect[] Customer

**Customerlf customer, please specify as below:**
**Platform:** unselect[] Omega (Do/Di 1.71) unselect[] Mega (Do/Di 1.55) unselect[] Alpha(Do/Di1.49)

**Type of process:** select[X] Premix unselect[] Split feed

**Expected throughput:** 400 kg/hr
**Screw diameter:**  mm
**Screw speed:**  rpm
**Motor:**  kW
**Motor type:** unselect[] Water cooled select[X] Air cooled unselect[] Synchronous servo motor
**Nominal torque:**  Nm/shaft
**Specific torque:**  Nm/cm3
**Gearbox make preferred:** unselect[] PIV unselect[] Zambello unselect[] Eisenbeiss unselect[] STEER

**Note:** - Gearbox cooling by default on right side look from die end
**Note:** - STEER gearbox available up to 60 mm extruders
**No of side feeder required: -**
select[X] 1 unselect[] 2 unselect[] 3 if more please specify _______
**Preferred L/D: -** _______

**Type of barrels: -** unselect[] 4D unselect[] 6D
**Maximum barrel temperature: -** 300 deg C

**Barrel, Element and Shaft MOC preference:** select[X] STEER unselect[] Customer specified
If customer specified please provide below details,

**Processing zone details,**

| Barrel Length*          |     |     |     |     |     |     |     |     |
| :----------------------- | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
| Barrel Type**            |     |     |     |     |     |     |     |     |
| Barrel preffered MOC*** |     |     |     |     |     |     |     |     |
| Element Preffered MOC***|     |     |     |     |     |     |     |     |
| Shaft MOC***             |     |     |     |     |     |     |     |     |

* 4D or 6D length
** 1-Intake, 2-Closed, 3-Side feed, 4-ATM Vent, 5-Vaccum Vent , 6-Liquid injection
*** Customer to specify required MOC

INDIA | CHINA| JAPAN | USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_7.png

```markdown
# STEER

# Extruder details

Co-rotating Twin screw extruder Enquiry Form

- **Drive and PLC panel cooling type:-**
    - unselect[] Fan cooled
    - unselect[] Air Condition
- **Cable entry to Drive and PLC panel:-**
    - select[X] Top
    - unselect[] Bottom
- **Cable exit from Drive and PLC panel:-**
    - select[X] Top
    - unselect[] Bottom
- **Note:** - Cable exit will be from the side of the panel for PLC panel supplied with plug and play type cables
- **Electrical STD preference if any please specify:**
- **Preferred operating interface: -**
    - unselect[] Basic push button type
    - select[X] HMI type
    - unselect[] Industrial PC type
- **Question:** Operating side when viewed from Die end: Left side
- **Question:** TCU position when viewed from Die end: Left side
- **Question:** Vacuum system when viewed from Die end: Left side
- **Question:** Side feeder when viewed from Die end: Left side
- **Please specify/attach additional information in case of customization required :**

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_8.png

```markdown
# STEER
Co-rotating Twin screw extruder Enquiry Form

# Other details

**Extruder maintenance resource:**
- select[X] In house
- unselect[] External/contract
- unselect[] NA

**Extruder maintenance resource:**
- unselect[] Unskilled
- select[X] Semi-skilled
- unselect[] Skilled
- unselect[] NA

**Type of training required from Extruder supplier:**
- unselect[] TSE operation
- unselect[] TSE maintenance
- unselect[] Application processing
- unselect[] NA

**Color coding:**
- select[X] Standard, Machine base, motor and Panels: RAL7035, Gearbox and feeders: RAL5009, All types of Gaurds: RAL1003,
- unselect[] Non Standard, please specify details ________________________________

**Extruder Manuals:**
- unselect[] Standard English
- unselect[] Other language, please specify ________________________________

# Project details

**Expected finalization time line:** ______ Weeks
**Expected delivery time:** Mar-25 ______ Weeks

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

