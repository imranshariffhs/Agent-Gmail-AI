# PDF Extraction Summary
                                **File:** /mnt/c/Users/Imran/OneDrive - Ahana Systems and Solutions (P) Ltd/Desktop/Demo/steer_document_processing_poc/demo_app/backend/download_email/enquiry_20250624_171144_TSE enquiry (Ningbo Licheng, Mega50).pdf
                                **Total Pages:** 8
                                **Successfully Processed:** 8
                                **Failed:** 0
                                **Extraction Date:** 2025-06-24 21:21:03
                                ---
                        ## page_1.png

```markdown
# Co-rotating Twin screw extruder Enquiry Form

SEPL/SYS-TSE-EF/F-01 Rev: No: 00 / 11-08-2020

## Please tell us about your company details

**Name of the client:** Ningbo Licheng
**Building No:** 
**Street:**
**City/Town:** Yuyao
**State:** Ningbo
**Pin code:**
**Telephone:**
**Email:**
**Website:**

## Site Information

**Ambient temperature at site:** Min 5 deg C Max 35 deg C
**Altitude:** 50 Meters above sea level
**Relative humidity at site:** Min 10 % Max 80 %
**Power voltage/Hz:** 380 Voltage 50 Hz
**3 Phase, 1 neutral:** Yes
**Cooling water available:** Yes
**Raw water available:** Yes
**Pneumatic air available:** Yes
**Place of machine installation:** Non hazardous
**Please specify IP protection required:** Standard IP55
unselect[] Main Motor
unselect[] Lube oil pump
unselect[] Small motors
unselect[] Junction box
unselect[] Heaters and Thermocouple
unselect[] PLC panel
unselect[] Drive panel

```

---

## page_2.png

```markdown
# STEER
Co-rotating Twin screw extruder Enquiry Form

# Scope of supply and work

select[X] Only extruder
unselect[] Any addition please select below,
**Extruder supplier scope:**
unselect[] Mezzanine floor
unselect[] Refill hoppers
unselect[] Conveying Gravimetric feeder
unselect[] Volumetric feeder
unselect[] Screen changer
unselect[] Melt pump
unselect[] Classifier
unselect[] Water bath
unselect[] Air knife Pelletizer
unselect[] Under water pelletizer
unselect[] FG storage and conveying
unselect[] 25Kg/Jumbo bag packing and sealing

**Extruder supplier work:** -
unselect[] Complete plant engineering
unselect[] Cable supply, laying and termination
unselect[] Upstream and downstream equipment supervision work
unselect[] Utility piping supply and connection
unselect[] De-dusting system and ducting work

Note: - Manpower and material handling resources need to be arranged by customer.

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_3.png

```markdown
# STEER
Co-rotating Twin screw extruder Enquiry Form

# Application details

**Basic recipe details:**:
PPA compounding

**End Application details:**
unselect[] Industrial application
select[X] Home appliance
unselect[] Electrical and electronics
unselect[] Automotive
unselect[] Housing and construction
select[X] Packaging
unselect[] Specialty

**End product usage:**
select[X] Extrusion
select[X] Molding

**End product form:**
select[X] Granules
select[X] Sheet

**End product testing method:**
unselect[] ASTM
select[X] ISO
unselect[] Will be provided before contract

**Bench mark available:**
unselect[] Yes
unselect[] No
unselect[] Will be provided before contract

**If yes, please specify:**
___MFI ___Impact ___Tensile ___Flexural ___ Flammability unselect[] Will be provided
before contract

**If any other, please specify:**
______________________________________________________

**Lab trial required:**
select[X] Yes unselect[] No

**If yes, please specify location for the lab trial:**
unselect[] STEER India
unselect[] STEER Japan
select[X] STEER china
unselect[] STEER America

**Willing to send sample along with MSDS to specified location:**
select[X] Yes
unselect[] No

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_4.png

```markdown
# STEER

Co-rotating Twin screw extruder Enquiry Form

# Raw material details

## Formulation-1

| SI | Raw material Name                       | Bulk density gram/cc | Physical form(Flakes/granules/powder/liquid) | Size in mm/micron/ Cps | Feed rate % |
|----|-----------------------------------------|----------------------|----------------------------------------------|-----------------------|-------------|
| 1  | PA 10T                                  |                      | POWDER                                      |                       | 44.5%       |
| 2  | Phosphorus nitrogen flame retardant     |                      | POWDER                                      |                       | 10%         |
| 3  | Chopped GF                              |                      |                                              |                       | 45%         |
| 4  | additive and carbon black               |                      | POWDER                                      |                       | 0.5%        |
| 5  |                                         |                      |                                              |                       |             |
| 6  |                                         |                      |                                              |                       |             |
| 7  |                                         |                      |                                              |                       |             |

## Formulation-2

| SI | Raw material Name | Bulk density gram/cc | Physical form(Flakes/granules/powder/liquid) | Size in mm/micron/ Cps | Feed rate % |
|----|-------------------|----------------------|----------------------------------------------|-----------------------|-------------|
| 1  |                   |                      |                                              |                       |             |
| 2  |                   |                      |                                              |                       |             |
| 3  |                   |                      |                                              |                       |             |
| 4  |                   |                      |                                              |                       |             |
| 5  |                   |                      |                                              |                       |             |
| 6  |                   |                      |                                              |                       |             |
| 7  |                   |                      |                                              |                       |             |

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

## page_5.png

```markdown
# STEER

Co-rotating Twin screw extruder Enquiry Form

# Raw material details

## Formulation-3

| SI | Raw material Name | Bulk density gram/cc | Physical form(Flakes/granules/powder/liquid) | Size in mm/micron/ Cps | Feed rate % |
|---|---|---|---|---|---|
| 1 |  |  |  |  |  |
| 2 |  |  |  |  |  |
| 3 |  |  |  |  |  |
| 4 |  |  |  |  |  |
| 5 |  |  |  |  |  |
| 6 |  |  |  |  |  |
| 7 |  |  |  |  |  |

**Raw material storage:**
- unselect[] Bulk silo
- unselect[] Inside warehouse

**Type of screening required:**
- unselect[] Magnetic separator
- unselect[] Metal separator

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

## page_6.png

```markdown
# STEER
## Co-rotating Twin screw extruder Enquiry Form

# Extruder details

**Extruder specification required from:** select[X] STEER unselect[] Customer

**Customerlf customer, please specify as below:**

**Platform:** unselect[] Omega (Do/Di 1.71) select[X] Mega (Do/Di 1.55) unselect[] Alpha(Do/Di1.49)

**Type of process:** unselect[] Premix select[X] Split feed

**Expected throughput:**  kg/hr

**Screw diameter:** 50 mm

**Screw speed:** 600 rpm

**Motor:** 132 kW

**Motor type:** unselect[] Water cooled unselect[] Air cooled unselect[] Synchronous servo motor

**Nominal torque:** Top Nm/shaft

**Specific torque:** Top_Nm/cm3

**Gearbox make preferred:** unselect[] PIV unselect[] Zambello unselect[] Eisenbeiss select[X] STEER

**Note:** - Gearbox cooling by default on right side look from die end

**Note:** - STEER gearbox available up to 60 mm extruders

**No of side feeder required:** -

unselect[] 1 select[X] 2 unselect[] 3  **if more please specify**

**Preferred L/D:** 52

**Type of barrels:** - unselect[] 4D unselect[] 6D

**Maximum barrel temperature:** - 400 deg C

**Barrel, Element and Shaft MOC preference:** select[X] STEER unselect[] Customer specified

**If customer specified please provide below details,**

# Processing zone details,

| Barrel Length* |    |    |    |    |    |    |    |    |
|---|---|---|---|---|---|---|---|---|
| Barrel Type** |    |    |    |    |    |    |    |    |
| Barrel preffered MOC*** | CR15 |    |    |    |    |    |    |    |
| Element Preffered MOC*** | CR4M |    |    |    |    |    |    |    |
| Shaft MOC*** |    |    |    |    |    |    |    |    |

* 4D or 6D length
** 1-Intake, 2-Closed, 3-Side feed, 4-ATM Vent, 5-Vaccum Vent , 6-Liquid injection
*** Customer to specify required MOC

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

## page_7.png

```markdown
# STEER
## Co-rotating Twin screw extruder Enquiry Form

# Extruder details

Drive and PLC panel cooling type:- unselect[] Fan cooled unselect[] Air Condition
Cable entry to Drive and PLC panel:- unselect[] Top unselect[] Bottom
Cable exit from Drive and PLC panel:- unselect[] Top unselect[] Bottom
Note: - Cable exit will be from the side of the panel for PLC panel supplied with plug and play type cables
**Electrical STD preference if any please specify:**

**Preferred operating interface: -**
select[X] HMI type
unselect[] Basic push button type unselect[] Industrial PC type

**Operating side when viewed from Die end:** Right side
**TCU position when viewed from Die end:** Right side
**Vacuum system when viewed from Die end:** Right side
**Side feeder when viewed from Die end:** Right side

Please specify/attach additional information in case of customization required :
1. side feeder, spare shaft with elements, vacuum stuffer quoted as optional
2. SCG may source side feeder locally if competitors give too low price

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA
STEER A NEW WORLD
```

---

## page_8.png

```markdown
# STEER

Co-rotating Twin screw extruder Enquiry Form

## Other details

**Extruder maintenance resource:**
- select[X] In house
- unselect[] External/contract
- unselect[] NA

**Extruder maintenance resource:**
- select[X] Unskilled
- unselect[] Semi-skilled
- unselect[] Skilled
- unselect[] NA

**Type of training required from Extruder supplier:**
- select[X] TSE operation
- select[X] TSE maintenance
- select[X] Application processing
- unselect[] NA

**Color coding:**
- select[X] Standard, Machine base, motor and Panels: RAL7035, Gearbox and feeders: RAL5009, All types of Gaurds: RAL1003,
- unselect[] Non Standard, please specify details ________________________________

**Extruder Manuals:**
- unselect[] Standard English
- select[X] Other language, please specify: Chinese

## Project details

**Expected finalization time line:** _______ Weeks
**Expected delivery time:** _______ Weeks

INDIA | CHINA| JAPAN| USA | EUROPE | SOUTH KOREA

STEER A NEW WORLD
```

---

